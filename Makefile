# This Makefile is for the Path::Extended::Tiny extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Path/Extended/Tiny.pm]
#     AUTHOR => [q[Kenichi Ishigaki <ishigaki@cpan.org>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => { ExtUtils::MakeMaker::CPANfile=>q[0.06] }
#     LICENSE => q[perl]
#     META_ADD => {  }
#     META_MERGE => { dynamic_config=>q[0], resources=>{ repository=>q[https://github.com/charsbar/Path-Extended-Tiny] } }
#     NAME => q[Path::Extended::Tiny]
#     PREREQ_PM => { Path::Tiny=>q[0], File::Find=>q[0], Test::UseAllModules=>q[0.10], Encode=>q[0], Test::More=>q[0.88], Text::Glob=>q[0], File::Path=>q[0], File::Copy::Recursive=>q[0] }
#     TEST_REQUIRES => { Test::More=>q[0.88], Test::UseAllModules=>q[0.10] }
#     VERSION_FROM => q[lib/Path/Extended/Tiny.pm]
...
MAKE = dmake
FULLEXT = Path\Extended\Tiny
BASEEXT = Tiny
PARENT_NAME = Path::Extended
DLBASE = $(BASEEXT)
...
INST_LIBDIR      = $(INST_LIB)\Path\Extended
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\Path\Extended
...
TO_INST_PM = lib/Path/Extended/Tiny.pm

PM_TO_BLIB = lib/Path/Extended/Tiny.pm \
	blib\lib\Path\Extended\Tiny.pm
...
DISTNAME = Path-Extended-Tiny
DISTVNAME = Path-Extended-Tiny-0.01
...
Tiny.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"Path::Extended::Tiny\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: 'a Path::Tiny wrapper for Path::Extended compatibility'" >> META_new.yml
	$(NOECHO) $(ECHO) author: >> META_new.yml
	$(NOECHO) $(ECHO) "  - 'Kenichi Ishigaki <ishigaki@cpan.org>'" >> META_new.yml
	$(NOECHO) $(ECHO) build_requires: >> META_new.yml
	$(NOECHO) $(ECHO) "  ExtUtils::MakeMaker: '0'" >> META_new.yml
	$(NOECHO) $(ECHO) "  Test::More: '0.88'" >> META_new.yml
	$(NOECHO) $(ECHO) "  Test::UseAllModules: '0.10'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: Path-Extended-Tiny" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "  Text::Glob: '0'" >> META_new.yml
	$(NOECHO) $(ECHO) resources: >> META_new.yml
	$(NOECHO) $(ECHO) "  repository: https://github.com/charsbar/Path-Extended-Tiny" >> META_new.yml
	$(NOECHO) $(ECHO) "version: '0.01'" >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) {{ > META_new.json
	$(NOECHO) $(ECHO) "   \"abstract\" : \"a Path::Tiny wrapper for Path::Extended compatibility\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"Kenichi Ishigaki ^<ishigaki^@cpan.org^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"Path-Extended-Tiny\"," >> META_new.json
	...
	$(NOECHO) $(ECHO) "            \"Test::UseAllModules\" : \"0.10\"" >> META_new.json
	$(NOECHO) $(ECHO) "         }}" >> META_new.json
	$(NOECHO) $(ECHO) "      }}," >> META_new.json
	...
	$(NOECHO) $(ECHO) "            \"Text::Glob\" : \"0\"" >> META_new.json
	$(NOECHO) $(ECHO) "         }}" >> META_new.json
	$(NOECHO) $(ECHO) "      }}" >> META_new.json
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	...
	$(NOECHO) $(ECHO) "         \"url\" : \"https://github.com/charsbar/Path-Extended-Tiny\"" >> META_new.json
	$(NOECHO) $(ECHO) "      }}" >> META_new.json
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) "    <ABSTRACT>a Path::Tiny wrapper for Path::Extended compatibility</ABSTRACT>" >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) "    <AUTHOR>Kenichi Ishigaki &lt;ishigaki@cpan.org&gt;</AUTHOR>" >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) "        <REQUIRE NAME=\"Text::Glob\" />" >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/Path/Extended/Tiny.pm blib\lib\Path\Extended\Tiny.pm 
